...
function Superfeedr(login, password, resource) {
  ...
  this.callbacks = {};
  this.client = new Client({ jid: this.jid, password: password });

  this.client.connection.socket.setTimeout(0);
  this.client.connection.socket.setKeepAlive(true, 10000);

  this.client.on('online', function() {
    self.client.send(new ltx.Element('presence', { }).c('show').t('chat').up().c('status').t(''));
    self.emit('connected');
  });
...
Superfeedr.prototype.subscribe = function(url, clb) {
  ...
  var subscribeStanza = new Core.Stanza.Iq({to: SUPERFEEDR_ENDPOINT, id:id, type:'set', from: this.client.jid})
...
Superfeedr.prototype.unsubscribe = function(url, clb) {
  var id = Math.floor(Math.random()*100000).toString();
  var unsubscribeStanza = new Core.Stanza.Iq({to: SUPERFEEDR_ENDPOINT, id:id, type:'set', from: this.client.jid})
  .c("pubsub", {xmlns: "http://jabber.org/protocol/pubsub"})
...
Superfeedr.prototype.list = function(page, clb) {
  ...
  var listStanza = new Core.Stanza.Iq({type:'get', from: this.client.jid, to: SUPERFEEDR_ENDPOINT, id:id})
...
Superfeedr.prototype.retrieve = function retrieve(url, clb) {
  var self = this;
  var id = Math.floor(Math.random()*100000).toString();
  var retrieveStanza = new Core.Stanza.Iq({to: SUPERFEEDR_ENDPOINT, id:id, type:'get', from: self.client.jid})
  .c("pubsub", {xmlns: "http://jabber.org/protocol/pubsub"})
